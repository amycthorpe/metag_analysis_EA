...
def main():
    ...
    parser.add_argument('-d', '--output-directory', required=True,
        dest='output_directory', help='Directory where the combined output file will be saved')
    args = parser.parse_args()

    # Process each file 
    samples = {}  # Map number to name
    sample_mapping = {}  # Create a mapping of input file names to sample names
    sample_count = 0
    ...
    for in_file in args.in_files:
        ...
        sample_count += 1
        sample_name = os.path.basename(in_file)
        sample_mapping[sample_name] = "Sample #" + str(sample_count)
        for line in i_file:
    ...
    # Write header
    combined_output_file_path = os.path.join(output_directory, "combined_output.tsv")
    o_file = open(combined_output_file_path, 'w')
    ...
    mapping_file_path = os.path.join(output_directory, "sample_mapping.tsv")
    with open(mapping_file_path, 'w') as mapping_file:
        for sample_name, mapped_name in sample_mapping.items():
            mapping_file.write(f"{sample_name}\t{mapped_name}\n")
